...
const BUILD_VERSION = "29.4-multisource-hybrid-market-intelligence-sync";
    ...
    try {
      const response = await fetch(url, {
        headers: {
          ...
          "User-Agent": "Investition-Dashboard/29.4 (+market monitor)",
        },
  ...
  try {
    ...
    try {
      ...
      syncWarnings.unshift(
        "Bewertungsschema fehlt; News wurden im Kompatibilitätsmodus gespeichert. setup-v29-4-consistent.sql ausführen.",
      );
    ...
    console.error("sync-news-v29-4", { requestId, message });
    if (admin) {
      try {
        await recordSyncRun(admin, {
          started_at: new Date(startedAt).toISOString(),
