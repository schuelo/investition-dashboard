const CACHE_NAME = 'investition-dashboard-v14';
const CORE = [
  './', './index.html', './app.js?v=14', './news.js?v=14', './supabase.js?v=14',
  './startdaten.json', './news-startdaten.json'
];
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(caches.match(request).then(cached => {
    const fresh = fetch(request).then(response => { if (response.ok) { const copy=response.clone(); caches.open(CACHE_NAME).then(cache => cache.put(request, copy)); } return response; });
    return cached || fresh;
  }));
});
