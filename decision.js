(() => {
  ...
  function computeHealth() {
    ...
    const health = [
      ...
      {name:'Entscheidungsschema', status:schemaMissing ? 'warn' : 'good', value:schemaMissing ? 'Schema prüfen' : 'Cloud aktiv', detail:schemaMissing ? 'setup-v29-4-consistent.sql ausführen; betroffene Module bleiben bis dahin eingeschränkt' : 'Thesen, Szenarien und Ereignisse werden synchronisiert'},
  ...
  function generateSignals() {
    ...
    state.news.filter(item => new Date(item.published_at || item.created_at).getTime() >= cutoff).forEach(item => {
      ...
      impacted.forEach(trade => {
        ...
        const fallbackImpact = impactLabel === 'hoch' ? 85 : impactLabel === 'niedrig' ? 35 : 60;
        const relevance = clamp(item.scores?.relevance ?? item.personal_relevance_score ?? item.relevance_score ?? 95);
        const confidence = clamp(item.scores?.confidence ?? item.confidence_score ?? 60);
        const impact = clamp(item.scores?.impact ?? item.impact_score ?? fallbackImpact);
        const urgency = clamp(item.scores?.urgency ?? item.urgency_score ?? (impactLabel === 'hoch' ? 75 : 45));
        signals.push(signal({
          key:`news:${item.id || item.external_id}:${trade.id}`,
          source_type:'news',
          source_id:item.id,
          trade_id:trade.id,
          symbol:trade.symbol,
          title:item.title,
          summary:item.relevance_reason || item.market_impact || item.summary || item.content || `Neue Meldung zu ${trade.name}.`,
          action:item.recommended_action || 'Auswirkung auf These, Bewertung und Kursreaktion einordnen.',
          relevance,
          confidence,
          impact,
          urgency
...
Kein Kauf-, Stop- oder KO-Entscheid auf Basis eines veralteten oder fehlgeschlagenen Kursabrufs. Der Zeitpunkt und die Quelle müssen sichtbar sein.</div></div>
      ...
      <div class="card decision-panel decision-span-12">${sectionHeader('Installationsstand', 'Ein konsolidierter V29.4-Stand')}<div class="table-shell-v25"><table><thead><tr><th>Baustein</th><th>Status</th><th>Referenz</th></tr></thead><tbody><tr><td>GitHub Dashboard</td><td><span class="chip good">29.4 Multi-Source</span></td><td>einheitliche Browser- und Cache-Version 29.4</td></tr><tr><td>Supabase Schema</td><td><span class="chip ${state.schemaReady?'good':'warn'}">${state.schemaReady?'erreichbar':'prüfen'}</span></td><td>setup-v29-4-consistent.sql</td></tr><tr><td>Edge Functions</td><td><span class="chip good">3 im Paket</span></td><td>sync-news, send-news-alerts, send-digest</td></tr><tr><td>Hintergrund-Automatik</td><td><span class="chip neutral">3 eindeutige Jobs</span></td><td>News :07 · Alarme :17 · Berichte alle 15 Minuten</td></tr><tr><td>Gesamtdiagnose</td><td><span class="chip neutral">nur lesend</span></td><td>diagnose-v29-4.sql</td></tr></tbody></table></div></div></div>`;
