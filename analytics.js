(() => {
  ...
  async function saveSnapshot() {
    if (!state.sb || !state.session) { setStatus('Bitte zuerst anmelden.', 'bad'); return; }
    if (!state.analyticsSchemaReady) { setStatus('Zuerst setup-v29-4-consistent.sql im Supabase SQL Editor ausführen.', 'bad'); return; }
    const m = metrics();
    const payload = {
      user_id: state.session.user.id,
  ...
  async function load() {
    ...
    const schemaNote = state.analyticsSchemaReady ? '' : ' · V29.4-Schema noch nicht vollständig installiert';
